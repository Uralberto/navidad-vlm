...
const botonPlay = document.querySelectorAll("button");
...
// Animación de imagenes del mosaico tipo uno
// Deformación y rotación de las imagenes del mosaico sin translación
...
// Animación de la letras F E L I Z   N A V I D A D

let color = 'green'
let letra = letras.length - 1;
let numero = numeros.length - 1

function animarCaracteres(caracter, i, t) {
  let n = i
  let sI = setInterval(function () {
    let moverX = Math.random() * imgHeight * .3
    let moverY = Math.random() * imgHeight * .1
    caracter[i].style.color = color
    caracter[i].style.transform = 'translate(' + moverX + 'px,' + -moverY + 'px)'
...
animarCaracteres(letras, letra, 10)
animarCaracteres(numeros, numero, 50)
...
function animacionUno() {
  if (controlUno === 0) {
    ...
    controlUno = 1
    botonPlay[0].innerHTML = '<i class="fas fa-pause"></i>'

  }
  else {
    controlUno = 0
    botonPlay[0].innerHTML = '<i class="fas fa-play"></i>'
  }
}
...
function animacionDos() {
  if (controlDos === 0) {
    ...
    controlDos = 1
    botonPlay[1].innerHTML = '<i class="fas fa-pause"></i>'

  }
  else {
    controlDos = 0
    botonPlay[1].innerHTML = '<i class="fas fa-play"></i>'
    item = img.length - 1
  }
}
...
function animacionTres() {
  if (controlTres === 0) {
    ...
    controlTres = 1
    botonPlay[2].innerHTML = '<i class="fas fa-pause"></i>'

  }
  else {
    controlTres = 0
    botonPlay[2].innerHTML = '<i class="fas fa-play"></i>'
    item = img.length - 1
  }
}
...
function animacionCuatro() {
  if (controlCuatro === 0) {
    ...
    controlCuatro = 1
    botonPlay[3].innerHTML = '<i class="fas fa-pause"></i>'

  }
  else {
    controlCuatro = 0
    botonPlay[3].innerHTML = '<i class="fas fa-play"></i>'
    item = img.length - 1
  }
}